...
class RoboMachine(object):
    ...
    def __init__(self, states, variables, rules, settings_table=None,
        ...
        print 'initialize robomachine'
        print self.variables[0].values
        #print self.variables[0].current_value
    ...
    def variable_value_mapping(self):
        #print  len(dict((v.name, v.current_value) for v in self.variables))
...
class State(object):
    ...
    def actions(self):
        result = []
        canidate_action_list =[]
        names = set()
        for action in self._actions:
            if action.is_available() and action.name not in names:
                #print action
                canidate_action_list.append(action)
                result.append(action)
                names.add(action.name)
        #print len(result)
        #print names
        #print '****'
        #print canidate_action_list
        #result.append(random.choice(canidate_action_list))
        #print result[0].name
        return result
...
class Action(object):
    ...
    def set_action_description(self,description):
