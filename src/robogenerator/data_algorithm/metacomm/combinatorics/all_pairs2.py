...
class all_pairs2:
    ...
    def __init__( self, options, filter_func = lambda x: True, previously_tested = [[]], n = 2 ):
        ...
        '''
        self.attrs = options.keys()
        for i in range(len(self.attrs)):
            self.__working_arr.append( [ item("a%iv%i" % (i,j), value) \
                                         for j, value in enumerate(options[self.attrs[i]] ) ] )
    ...
    def next( self ):
        ...
        while -1 < i < len(self.__working_arr):
            ...
            dict_row ={}
            element_row = self.get_values_array( chosen_values_arr[:i+1] )
            if len(element_row) == len(self.attrs):
            	for attr in self.attrs:
            		dict_row[attr] = element_row[self.attrs.index(attr)]
            if self.__filter_func( dict_row ):
                assert(direction > -1)
                direction = 1
            else:
                direction = 0
