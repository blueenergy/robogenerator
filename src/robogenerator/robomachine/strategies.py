...
class DepthFirstSearchStrategy(_Strategy):
    ...
    def _variable_value_sets(self, variables):
            if not variables:
                return ([],)
            #print (vs for vs in self._var_set(variables) if self._machine.rules_are_ok(vs))
            return (vs for vs in self._var_set(variables) if self._machine.rules_are_ok(vs))

    def _var_set(self, vars):
        if not vars:
            return [[]]
        #print self._var_set(vars[1:])
        #print vars[0].values
        for xxx in ([val]+sub_set for val in vars[0].values for sub_set in self._var_set(vars[1:])):
            print xxx
        print 'END'
        ...
        if not state.actions or max_actions == 0:
            ...
            for action in state.actions:
                for test in self._generate_all_from(action.next_state, max_actions-1):
                    ...
                    description = '  '.join(self._values)
                    action.set_action_description('%s'%description)
                    yield [action]+test
...
class RandomStrategy(_Strategy):
    ...
    def tests(self):
    	print 'we are generating random tests'

        while True:
            test = self._generate_test()
            #print self._machine.start_state.name
            #print self._to_state
    ...
    def _generate_test(self):
        test = []
        value = self._generate_variable_values()
        self._machine.apply_variable_values(value)
